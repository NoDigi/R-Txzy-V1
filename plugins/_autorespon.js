    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'LyXzy tidak aktif' : banned ? 'kamu dibanned' : 'LynXzy disini🗿',
                '©LynXzy',
...
╠═〘 LYNXZY 〙 ═`.trim(), '© LynXzy', 'Gopay', '#viadana', 'Pulsa', '#viapulsa', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`Aktif selama ${uptime} | Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik'} |LynXzy🌸 by Lyn Official🌕`).catch(_ => _)
